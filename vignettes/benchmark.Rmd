...
vignette: >
  ...
  %\VignetteDepends{dplyr,tidyr,bench}
---
...
Several tidyverse packages are used to post-process and reshape the benchmark results:
...
The exported object `flower` is a `reeb_graph` constructed from the flower mesh, one of 12 from the `AIM@SHAPE` collection used by Tu &al to benchmark algorithms.
Among them, it contains the most critical points, so it will better illustrate the difference between the algorithms in computationally intensive settings.
...
So that the benchmark results are more reflective of the algorithms' requirements, we manually drop these points first:
...
For this Reeb graph, Propagate and Pair outperforms the multi-pass algorithm by roughly a factor of 3.

## How performance and improvement scale

Finally, we compare how both algorithms scale.
Vertex and edge data are provided for three random merge tree Reeb graphs on an exponential size scale.
These are converted to split trees by negating the vertex values and benchmarked separately before being stacked. (Note that the single-pass algorithm was found superior to the multi-pass algorithm on split trees but inferior on merge trees.)

```{r random (split) trees benchmark}
...
```{r random (split) trees plot, fig.width=8, fig.height=8, out.width="100%"}
# prepare runtime data for plotting
plot_data <- tree_bench %>%
  select(size, method, time) %>%
  unnest(time)
plot_data$time_ms <- as.numeric(plot_data$time) * 1e3

# grouped boxplot with base graphics
sizes <- sort(unique(plot_data$size))
cols <- c(multi = "#E41A1C", single = "#377EB8")
boxplot(
  log1p(time_ms) ~ method + as.factor(size),
  data = plot_data,
  xlab = "tree size",
  ylab = "run time (1 + log ms)",
  border = cols, col = paste0(cols, "77"),
  xaxt = "n"
)
axis(1, at = seq_along(sizes) * 2 - 0.5, labels = sizes)
legend("topleft", legend = names(cols), fill = cols, title = "method")
```

The single-pass advantage appears to grow with size.
The ratio of medians quantifies this:

```{r random (split) trees medians}
tree_bench %>%
